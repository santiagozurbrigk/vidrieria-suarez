-- Reordena los parámetros de las RPC para que los opcionales queden al final.
--
-- Motivo: `supabase gen types` marca como opcional (`p_x?: string`) sólo a los
-- parámetros que tienen DEFAULT, y declara al resto como no-nulos aunque
-- PostgreSQL acepte NULL en cualquiera. Con las firmas originales, TypeScript
-- rechazaba pasar `null` en campos que legítimamente pueden serlo —
-- `p_cliente_id` de un presupuesto sin cliente, por ejemplo.
--
-- PostgreSQL exige que todos los parámetros posteriores a uno con DEFAULT
-- también tengan DEFAULT, así que hay que reordenarlos. Como las llamadas van
-- por nombre (PostgREST usa notación nombrada), el reordenamiento no afecta a
-- ningún llamador.
--
-- Cambiar el orden o el nombre de los parámetros no se puede hacer con
-- CREATE OR REPLACE: hay que borrar la versión anterior primero.

drop function if exists crear_factura_venta(uuid, date, text, numeric, numeric, numeric, text, jsonb, text);
drop function if exists crear_factura_compra(uuid, date, text, numeric, numeric, numeric, text, jsonb, text);
drop function if exists crear_presupuesto(uuid, uuid, text, date, int, text, jsonb, text);
drop function if exists registrar_pago(text, uuid, uuid, numeric, text, date, text, jsonb);
...
begin
  ...
  insert into facturas_venta (
    ...
    p_cliente_id,
    coalesce(nullif(btrim(p_numero), ''), siguiente_numero('FACTURA_VENTA')),
    p_fecha,
  ...
  -- El trigger de stock corre por cada ítem y aborta la transacción entera si
  -- no alcanzan las existencias.
...
begin
  ...
  select margen_ganancia into v_margen from proveedores where id = p_proveedor_id;
  if not found then
  ...
  insert into facturas_compra (
    ...
    p_proveedor_id,
    coalesce(nullif(btrim(p_numero), ''), siguiente_numero('FACTURA_COMPRA')),
    p_fecha,
    coalesce(nullif(btrim(p_tipo_comprobante), ''), 'FACTURA'),
    p_subtotal,
    coalesce(p_iva, 0),
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    -- Los productos nuevos se crean dentro de esta misma transacción: si la
    -- factura falla después, no quedan huérfanos en el catálogo.
...
begin
  ...
  -- El total lo calcula el trigger sobre presupuesto_items: se relee la fila
  -- ya consolidada.
...
begin
  ...
  -- El trigger fn_caja_por_pago genera el movimiento de caja acá.
  ...
  for v_imp in select * from jsonb_array_elements(coalesce(p_imputaciones, '[]'::jsonb)) loop
    -- Se bloquea la factura mientras se valida el saldo, para que dos pagos
    -- simultáneos no la dejen en negativo.
...
-- ── Funciones que llaman a las anteriores ────────────────────────────────────
-- Se recrean porque usan notación nombrada y las firmas cambiaron.
...
begin
  ...
  -- Sólo los ítems con producto pueden ir a una factura: la factura descuenta stock.
  select jsonb_agg(jsonb_build_object(
   ...
   where presupuesto_id = p_presupuesto_id and producto_id is not null;
  ...
  update presupuestos
     set convertido_en_factura_id = v_factura.id, estado = 'CONVERTIDO'
...
declare
  v_factura facturas_venta;
begin
  select * into v_factura from facturas_venta where id = p_factura_id for update;
  if not found then
    raise exception 'Factura no encontrada';
  end if;
  ...
  if p_monto > v_factura.saldo_pendiente + 0.01 then
    raise exception 'El monto (%) supera el saldo pendiente (%)', p_monto, v_factura.saldo_pendiente;
  end if;
...
grant execute on function crear_factura_venta(uuid, date, numeric, numeric, jsonb, text, numeric, text, text) to authenticated;
grant execute on function crear_factura_compra(uuid, date, numeric, numeric, jsonb, text, numeric, text, text) to authenticated;
grant execute on function crear_presupuesto(uuid, date, jsonb, uuid, text, int, text, text) to authenticated;
grant execute on function registrar_pago(text, numeric, text, date, uuid, uuid, text, jsonb) to authenticated;
